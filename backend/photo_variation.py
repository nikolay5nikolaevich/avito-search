...
# Зум-приближение: шаг 0,5%, диапазон 15..24,5 — 20 значений, хватает для
# двадцати изменённых вариантов. Минимум 15% = центральный кроп 7.5%
...
_ZOOM_IN_VALUES: list[float] = [15.0 + index * 0.5 for index in range(20)]
...
def build_presets(n: int, seed: Optional[int] = None) -> list[Preset]:
    ...
    :param n: количество пресетов (1..21, включая нейтральный первый).
    :param seed: инициализация RNG (None → случайный).
    :raises ValueError: если n < 1 или n > 21.
    """
    if n < 1 or n > 21:
        raise ValueError(f"n должно быть в диапазоне 1..21, получено: {n}")
    ...
    # При n-1 <= 20 шаги гарантированно попарно различны.
    ...
    if len(presets) < n:
        ...
        # Крайний защитный fallback: штатно до этого не доходит.
        while len(presets) < n:
...
def build_modified_presets(n: int, seed: Optional[int] = None) -> list[Preset]:
    """Вернуть n заметно изменяющих фото пресетов без нейтрального оригинала.

    ``build_presets`` сохраняет прежний контракт с нейтральным пресетом №1.
    Подготовка объявлений использует эту функцию, потому что каждый черновик,
    включая первый, должен быть самостоятельным изменённым вариантом.
    """
    if n < 1 or n > 20:
        raise ValueError(f"n должно быть в диапазоне 1..20, получено: {n}")

    effective_seed = seed if seed is not None else random.randrange(2**32)
    return build_presets(n + 1, seed=effective_seed)[1:]
...
if __name__ == "__main__":
    ...
    # ── Тест 13: каждый пресет 2..21 содержит зум-приближение и тон ──────────
    for _seed in (1, 7, 42, 99):
        _ps = build_presets(21, seed=_seed)
    ...
    print("[OK] Тест 13: пресеты 2..21 всегда zoom_pct >= 15.0 + тон (4 seed)")

    # ── Тест 13б: шаги зума у пресетов 2..N попарно различны (n=2..21) ───────
    for _n in range(2, 22):
        for _seed in (1, 7, 42, 99):
    ...
    print("[OK] Тест 13б: шаги зума у пресетов 2..N попарно различны (n=2..21, 4 seed)")
